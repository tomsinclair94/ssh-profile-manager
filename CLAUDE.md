...
**Status:** Setup Complete (2025-01-08)

Private development files (CLAUDE.md, TODO.md, plans/) are gitignored from public repo but backed up to private repo using two-remote approach:

- **Public:** `origin` (github.com/tomsinclair94/ssh-profile-manager)
- **Private:** `private` (github.com/tomsinclair94/ssh-profile-manager-private)

The `all-files` branch in private repo contains complete development documentation for backup and multi-machine sync.

### Backup Workflow (CRITICAL - Run at Checkpoints)

**IMPORTANT:** Always backup private files to the private repo at these checkpoints:
- ✅ End of work session (user says "save" or "ready for next time")
- ✅ After completing a release
- ✅ After major milestone or significant changes to CLAUDE.md/TODO.md
- ✅ Before taking a break from the project

**Commands to backup:**
```bash
git checkout all-files
git merge main                           # Bring in latest code
git add -f CLAUDE.md TODO.md plans/     # Force-add gitignored files
git commit -m "Backup private files - [description]"
git push private all-files
git checkout main                        # Return to working branch
```

**Note:** This ensures development documentation is never lost and can be accessed from other machines.
