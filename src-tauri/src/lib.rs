...
fn connect_ssh(
    ...
    // On Windows: poll for the status file written by the terminal script on SSH auth failure.
    ...
    // The thread stops early if the bat writes OK (clean session close) or FAIL (auth failure).
    #[cfg(target_os = "windows")]
