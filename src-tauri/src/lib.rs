...
#[cfg(test)]
mod tests {
    /// Encryption unit tests — password validation, key derivation, HMAC
    /// integrity, encrypt/decrypt round-trips, tampering detection, algorithm
    /// rejection, and the helper functions used by import/export commands.
    ///
    /// Phase 8 will add sibling modules (profiles, groups, tags, …) here
    /// using an in-memory SQLite database for DB-layer coverage.
    mod encryption {
        use super::super::{
            decrypt_data,
